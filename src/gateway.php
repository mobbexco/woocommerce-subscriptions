...
class Gateway
{
    ...
    /* @var \Mobbex\WP\Subscriptions\Model\Subscription */
    public $subscription;

    /* @var \Mobbex\WP\Subscriptions\Model\Subscriber */
    public $subscriber;

    /* @var  \Mobbex\WP\Subscriptions\Model\Cart */
    public $cart;
    ...
    public function __construct()
    {
        ...
        $this->cart         = new \Mobbex\WP\Subscriptions\Model\Cart;
        $this->cart_helper  = new \Mobbex\WP\Subscriptions\Helper\Cart;
        $this->helper       = new \Mobbex\WP\Subscriptions\Model\Helper();
        $this->order_helper = new \Mobbex\WP\Subscriptions\Helper\Order();
        $this->subscriber   = new \Mobbex\WP\Subscriptions\Model\Subscriber;
        $this->subscription = new \Mobbex\WP\Subscriptions\Model\Subscription();
        ...
        $this->send_subscriber_email = $this->config->send_subscriber_email === 'yes';

        $this->error = false;

        if (!$this->helper->is_ready()) {
            $this->error = true;
            $this->logger->notice(__('You need to specify an API Key and an Access Token.'), 'error', 'Mobbex Subscriptions');
        }
        ...
        // Checkout filters
        if ($this->cart->has_subscription())
            add_filter('mobbex_checkout_custom_data', [$this, 'process_subscription'], 10, 2);

    }

    /**
     * Modify checkout data to add subscription functionality.
     * Get subscription and subscriber then return properly modified checkout data
     * 
     ...
     * @return 
     */
    public function process_subscription($checkout_data, $order_id)
    {
        global $woocommerce;

        if ($this->config->enable_subscription != 'yes' && !file_exists( MOBBEX_SUBS_DIRECTORY . 'Init.php'))
        ...
        if ($this->helper->is_subs_change_method()) {
            ...
            return [
                ...
                'redirect' => $result === false || is_wp_error($result) ? $this->helper::_redirect_to_cart_with_error('Error al intentar realizar el cobro de la suscripción') : $order->get_checkout_order_received_url(),
            ];
        } else {
            $subscription = $this->get_subscription($order);
            // $subscriber   = $this->get_subscriber($order, $subscription->uid);
        ...
        if (!empty($subscriber->uid) /*&& !empty($subscription->uid)*/) {
            $checkout_data['total']                   = 0;
            // $checkout_data['customer']['uid']         = $subscriber->reference;
            $checkout_data['items']['0']['reference'] = $subscription->uid;
            $checkout_data['items']['0']['type']      = 'subscription';
            return $checkout_data;
        }
    ...
    public function process_webhook($token, $data, $type, $id)
    {
        ...
        $subscription = $this->subscription::get_by_uid($data['subscription']['uid']);
        $subscriber   = $this->subscriber::get_by_uid($data['subscriber']['uid']);
        ...
        } else if ($this->cart::has_subscription($order_id)) {
    ...
    public function get_subscription($order)
    {
        ...
        foreach ($order->get_items() as $item) {
            ...
            if (SubscriptionProduct::is_subscription($product_id)) {
                    $sub_options['interval']   = implode(SubscriptionProduct::get_charge_interval($product_id));
                    $sub_options['trial']      = SubscriptionProduct::get_free_trial($product_id)['interval'];
                    $sub_options['signup_fee'] = SubscriptionProduct::get_signup_fee($product_id);
            }

            if ($this->helper->is_wcs_active() && !\WC_Subscriptions_Product::is_subscription($product) && !$this->cart::has_subscription($order_id)) {
                apply_filters('simple_history_log', __METHOD__ . ": Order #$order_id does not contain a any Subscription", null, 'error');
                return;
            }
        }

        $subscription = $this->subscription::create_mobbex_subscription($sub_options);
    ...
    public function get_subscriber($order, $mbbx_subscription_uid)
    {
        ...
        // Create subscriber
        $subscriber = new \Mobbex\WP\Subscriptions\Model\Subscriber(
            $order_id,
    ...
    public function scheduled_subscription_payment($total, $order)
    {
        ...
        if(!empty($subscription->uid))
            $subscriber = new \Mobbex\WP\Subscriptions\Model\Subscriber($wcs_sub->order->get_id());
    ...
    public function update_subscriber_state($subscription)
    {
        try {
            ...
            // Get susbscriber
            $subscriber = new \Mobbex\WP\Subscriptions\Model\Subscriber($order_id);
