...
use \Mobbex\WP\Subscriptions\Model\Cart as Cart;
use \Mobbex\WP\Subscriptions\Model\SubscriptionProduct as SubscriptionProduct;
use \Mobbex\WP\Subscriptions\Model\Subscription as Subscription;
use \Mobbex\WP\Subscriptions\Model\Subscriber as Subscriber;
/**
...
class Order
{
    ...
    public function get_post_id($product_id, $order)
    {
        if ($order && $this->helper->is_wcs_active()) {
            ...
            return Subscription::is_stored($wcs_sub->order->get_id()) ? $wcs_sub->order->get_id() : $product_id;
        } else {
            return $product_id;
        }
    }
