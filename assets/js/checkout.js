jQuery(function ($) {
  ...
  function executePayment() {
    ...
    if ($('[name=payment_method]:checked').val() != 'mobbex_subs') return;

    // If it is a sub change, use form submit
    if (mobbex_subs_data.is_subs_change) return;

    processOrder((response) =>
      response.redirect ? redirect(response) : openModal(response)
    );
  ...
  function processOrder(callback) {
    ...
    $.ajax({
      dataType: 'json',
      method: 'POST',
      url: mobbex_subs_data.is_pay_for_order
        ? form[0].action
    ...
    let options = {
      ...
      onResult: (data) => {
        window.top.location.href =
          response.return_url + '&status=' + data.status.code;
      },
      onClosePressed: () => {
  ...
  function renderLock() {
    if (!$('#mbbx-loader-modal').length)
      $('body').append(
        '<div id="mbbx-loader-modal" style="display: none;"><div id="mbbx-spinner"></div></div>'
      );
  }
